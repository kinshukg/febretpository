...
class LegendButton
{
  ...
  public LegendButton(int x, int y, String label, color c)
  {
    this.x = x;
    this.y = y;
    this.label = label;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// View config.
float xofs = float(width) * 0.05;
float yofs = float(height) * 1.50;
float zoom = 1;
float angle = PI + 0.02;
// Plot stuff.
float yScale;
...
void initMapData()
{
  ...
  while(i < streetDataLines.length)
  {
    ...
    streetData[j].id = int(args1[0]);
    if(streetData[j].id != 1) streetData[j].id = i;
    streetData[j].name = streetName(streetData[j].id);
    streetData[j].segs = new float[int(args1[1]) * 2];
    
    for(int k = 0; k < int(args1[1]); k++)
    {
      String[] args2 = split(streetDataLines[i++], '\t');
      float x = 1 - float(args2[0]) / 20 * mapSize;
      float y = float(args2[1]) / 20 * mapSize;
...
void initDeaths()
{
  ...
  while(i < deathLines.length)
  {
    ...
    float x = 1 - float(args[0]) / 20 * mapSize;
    float y = float(args[1]) / 20 * mapSize;
    deathData[i].x = x * cos(angle) - y * sin(angle);
    deathData[i].y = y * cos(angle) + x * sin(angle);
    deathData[i].age = int(args[2]);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
color maleColor = color(140, 140, 255);
color femaleColor = color(255, 50, 255);
color age1Color = color(250, 250, 0);
color age2Color = color(250, 200, 0);
color age3Color = color(250, 150, 0);
color age4Color = color(250, 100, 0);
color age5Color = color(250, 50, 0);
color age6Color = color(250, 0, 0);
...
void initPlotData()
{
  yScale = 1 / float(maxDeaths);
  xScale = new float[deathDays.length];
  for(int i = 0; i < deathDays.length; i++)
  {
    xScale[i] = 1 / float(deathDays.length);
  } 
}
...
void drawMap()
{
  ...
  for(int i = 0; i < streetNum; i++)
  {
    ...
    {
      x2 = int(streetData[i].segs[j] + xofs);
      y2 = int(streetData[i].segs[j + 1] + yofs);
...
void drawDeaths()
{
  ...
  for(int i = 0; i < deathData.length; i++)
  {
    ...
    {
      int x = int(deathData[i].x + xofs);
      int y = int(deathData[i].y + yofs);
...
void drawPumps()
{
  ...
  {
    int x = int(pumpData[i].x + xofs);
    int y = int(pumpData[i].y + yofs);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void drawPlotBase(int x, int y, int width, int height, String x1, String x2, String x3, String y1, String y2, String y3, String label)
{
  textFont(fnt, 14);
...
void drawDeathSexPlot()
{
  ...
  if(dayLength > 0)
  {
    ...
    {
      float ptValue = deathSexPlotY + plotHeight - float(dayData[i].m + dayData[i].f) * yScale * plotHeight;
      vertex(curX, ptValue);
      curX += float(plotWidth) * xScale[i];
    }
  }
  else
  {
    float ptValue = deathSexPlotY + plotHeight - float(dayData[dayStart].m + dayData[dayStart].f) * yScale * plotHeight;
  ...
  if(dayLength > 0)
  {
    for(int i = 0; i < dayData.length; i++)
    {
      ...
      float ptValue = deathSexPlotY + plotHeight - float(dayData[i].f) * yScale * plotHeight;
      vertex(curX, ptValue);
      curX += float(plotWidth) * xScale[i];
    }
  }
  ...
  {
    float ptValue = deathSexPlotY + plotHeight - float(dayData[dayStart].f) * yScale * plotHeight;
    vertex(deathSexPlotX, ptValue);
    vertex(deathSexPlotX + plotWidth, ptValue);
  }
  vertex(deathSexPlotX + plotWidth, deathSexPlotY + plotHeight);
  ...
  for(int j = 0; j < 6; j++)
  {
    ...
    if(dayLength > 0)
    {
      for(int i = 0; i < dayData.length; i++)
      {
        ...
        float ptValue = deathAgePlotY + plotHeight - float(val) * yScale * plotHeight;
        vertex(curX, ptValue);
        curX += float(plotWidth) * xScale[i];
      }
    }
...
void drawTimeline()
{
  ...
  float ix = float(winWidth) / deathDays.length;
  float iy = float(winHeight) / maxDeaths;
...
void mouseDragged()
{
  ...
  else
  {
    ...
    {
      zoom += float(pmouseY - mouseY) / height;
    }
  }
}
